...
public class SiegeBattleSolveStrategy(IUnitMatchupTable unitMatchupTable) : IBattleSolver
{
    public float AttackerCasualtiesPenaltyPercent { get; private set; } = 50.0f;
    public float AdditionalDefenderBonus { get; private set; } = 1.5f;
    ...
    public BattleReport ResolveBattle(Army attacker, Army defender)
    {
        ...
        var defenderPower = ArmyMath.CalculatePower(defender.UnitsCount, AdditionalDefenderBonus);

        if (attackerPower > defenderPower)
        {
            var defenderInitialUnits = defender.UnitsCount;
            var army1Casualties = ArmyMath.CalculateRandomCasualties(attacker.UnitsCount, AttackerCasualtiesPenaltyPercent);
